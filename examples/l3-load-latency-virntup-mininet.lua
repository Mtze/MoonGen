...
function configure(parser)
	...
	parser:option("-r --rate", "Transmit rate in kbit/s."):default(1000):convert(tonumber)
...
function master(args)
	...
	txDev = device.config{port = args.txDev, rxQueues = 3, txQueues = 3, disableOffloads = true}
	-- txDev = device.config{port = args.txDev, rxQueues = 3, txQueues = 3}
	rxDev = device.config{port = args.rxDev, rxQueues = 3, txQueues = 3}
	device.waitForLinks()

	local rateLimiter = limiter:new(txDev:getTxQueue(0),"cbr", 1 / args.rate * 1000000)  -- Supply delay in ns! 
	mg.startTask("loadSlave", txDev:getTxQueue(0), rxDev, args.size, args.flows, args, rateLimiter)
...
function loadSlave(queue, rxDev, size, flows, args, rateLimiter)
